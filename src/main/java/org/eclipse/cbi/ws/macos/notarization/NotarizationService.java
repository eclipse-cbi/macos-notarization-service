...
public class NotarizationService {
    ...
    public Response notarize(MultipartFormDataInput input) throws IOException, ServletException {
        MultipartFormDataInputWrapper.WithMoshi formData = new MultipartFormDataInputWrapper.WithMoshi(input, moshi);
	...
	private NotarizationStatusWithUUID notarize(MultipartFormDataInputWrapper.WithMoshi formData, InputStream file,
			NotarizationRequestOptions options) throws IOException {
		Path fileToNotarize = createTempFile(Paths.get(pendingFilesPath),
		        formData.submittedFilename("file").orElse("unknown"));

		copy(file, fileToNotarize);
		
		NotarizationRequest.Builder requestBuilder = NotarizationRequest.builder()
		...
		requestBuilder.notarizationInfo((NotarizerResult r) -> NotarizationInfo.builder()
			    			...
			    			.build()
			    			.retrieveInfoFailsafe(infoPollingMaxTotalDuration, infoPollingDelayBetweenSuccessfulAttempts, infoPollingMaxFailedAttempts, infoPollingMinBackOffDelay, infoPollingMaxBackOffDelay));
	...
	private static void copy(InputStream source, Path target) throws IOException {
		try (Source a = Okio.buffer(Okio.source(source));
		        BufferedSink b = Okio.buffer(Okio.sink(target))) {
		    b.writeAll(a);
		}
	}
    ...
    static Path createTempFile(Path parentFolder, String templateFilename)
            throws IOException {
