...
class MultipartFormDataInputWrapper {
  ...
  Optional<String> submittedFilename(String partName) {
    ...
    if (part.isPresent()) {
      ...
      if (!Strings.isNullOrEmpty(cdHeader)) {
        List<String> filenames = Splitter.on(';').trimResults().omitEmptyStrings().splitToList(cdHeader).stream()
            .filter(n -> n.startsWith("filename"))
            .map(n -> Splitter.on('=').trimResults().omitEmptyStrings().splitToList(n)).filter(l -> l.size() > 1)
